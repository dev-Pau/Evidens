...
final class SubSettingKindTests: XCTestCase {
    ...
    func testPasswordImage() {
        let subSettingKind = SubSettingKind.password
        let expectedImage = UIImage(systemName: AppStrings.Icons.key, withConfiguration: UIImage.SymbolConfiguration(weight: .medium))?.withRenderingMode(.alwaysOriginal).withTintColor(K.Colors.primaryGray)
        XCTAssertEqual(subSettingKind.image, expectedImage)
    }
    ...
    func testDeactivateImage() {
        let subSettingKind = SubSettingKind.deactivate
        let expectedImage = UIImage(named: AppStrings.Assets.brokenHeart)?.withRenderingMode(.alwaysOriginal).withTintColor(K.Colors.primaryGray)
        XCTAssertEqual(subSettingKind.image, expectedImage)
    }
}
