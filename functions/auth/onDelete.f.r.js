...
exports.releaseAuthOnDelete = functions.region('europe-west1').auth.user().onDelete(async (user) => {
    const userId = user.uid;

    try {
        console.log(`Deleting for user data for UID ${userId} started...`);
...
async function removeStorageImages(userId, folderPath) {
    ...
    try {
        ...
        console.error(`Error removing files for user ${userId}.`, error);
    }
}

async function removeUserData(userId) {
    await admin.database().ref(`users/${userId}`).remove();
    console.log(`Data for user ${userId} successfully deleted from Database.`);
}
...
async function removeUserDocument(userId) {
    ...
    try {
        ...
        console.log(`Fields for user ${userId} successfully updated from Firestore.`);
    } catch (error) {
