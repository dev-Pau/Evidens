//
//  ConversationMenuLauncher.swift
//  Evidens
//
//  Created by Pau Fernández Solà on 29/9/22.
//
...
protocol ConversationBottomMenuLauncherDelegate: AnyObject {
    func didTapDeleteConversation()
}


class ConversationMenuLauncher: NSObject {
    ...
    weak var delegate: ConversationBottomMenuLauncherDelegate?
    ...
    private var menuOptionsText: [String] = ["Delete conversation"]
    private var menuOptionsImages: [UIImage] = [UIImage(systemName: "trash", withConfiguration: UIImage.SymbolConfiguration(weight: .medium))!]
    ...
    @objc func handleDismiss(selectedOption: String?) {
        UIView.animate(withDuration: 0.5, delay: 0, usingSpringWithDamping: 2, initialSpringVelocity: 1, options: .curveEaseOut) {
            ...
            case "Delete conversation":
                self.delegate?.didTapDeleteConversation()
            default:
                break
            }
        }
...
extension ConversationMenuLauncher: UICollectionViewDelegateFlowLayout, UICollectionViewDataSource, UICollectionViewDelegate {
