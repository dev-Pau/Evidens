...
class MessageViewModel {
    ...
    var phase: String {
        ...
        case .sent:
            return ""
        case .sending:
            return AppStrings.Characters.dot + AppStrings.Content.Message.sending + "..."
        case .failed:
            return AppStrings.Characters.dot + AppStrings.Content.Message.failed
        case .unread:
            return ""
        }
    }
    ...
    func formatDateString(for date: Date) -> String {
        ...
        if isToday {
            formatter.dateFormat = "'" + AppStrings.Content.Message.today + "'"
            return formatter.string(from: date)
        } else if isYesterday {
            formatter.dateFormat = "'" + AppStrings.Content.Message.yesterday + "'"
            return formatter.string(from: date)
