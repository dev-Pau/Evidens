...
class SearchJobsViewController: UIViewController {
    ...
    private func getMoreJobs() {
        JobService.fetchJobsWithText(searchedText, lastSnapshot: lastJobSnapshot) { snapshot in
...
extension SearchJobsViewController: UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if !isInSearchMode {
            ...
            if jobs.isEmpty {
                ...
                cell.set(withImage: UIImage(named: "message.empty")!, withTitle: "No results for \"\(searchedText)\"", withDescription: "The term you entered did not bring up any results. You may want to try using different search terms.", withButtonText: "   Post a job   ")
                return cell
            } else {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        if !isInSearchMode {
            ...
            if jobs.isEmpty { return CGSize(width: view.frame.width, height: UIScreen.main.bounds.height * 0.6) }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if !isInSearchMode {
            ...
            fetchJobsWithSearchedText(searchedText)
            isInSearchMode = true
#warning("pass the data into the previous controller to display the text tapped in the bar")
