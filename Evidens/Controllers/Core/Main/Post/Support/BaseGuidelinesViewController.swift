...
class BaseGuidelinesViewController: UIViewController {
    ...
    private func createLayout() -> UICollectionViewCompositionalLayout {
        let layout = StretchyCompositionalLayout { [weak self] sectionNumber, env in
            ...
            let group = sectionNumber == 7 ? NSCollectionLayoutGroup.horizontal(layoutSize: NSCollectionLayoutSize(widthDimension: .fractionalWidth(0.7), heightDimension: .fractionalWidth(0.5)), subitems: [item]) : NSCollectionLayoutGroup.vertical(layoutSize: itemSize, subitems: [item])
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        if kind == ElementKind.sectionHeader {
            if indexPath.section == 0 {
                ...
                header.set(title: AppStrings.Guidelines.Case.title)
                return header
                ...
                header.set(title: AppStrings.Guidelines.Case.summary)
                return header
            } else if indexPath.section == 7 {
                let header = collectionView.dequeueReusableSupplementaryView(ofKind: kind, withReuseIdentifier: guidelineTitleHeaderReuseIdentifier, for: indexPath) as! BaseGuidelineTitleHeader
